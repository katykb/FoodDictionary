...
if (localStorage.getItem("storedItems")) {
    //gets item from local storage and stores into an array to access the values.
    storedFoodArray = JSON.parse(localStorage.getItem("storedItems"));
}
for (let i = 0; i < storedFoodArray.length; i++) {
    let nutrients = storedFoodArray[i].nutrientInfo;
    ...
    var protein = nutrients.find((n) => n.nutrientName === "Protein");
    ...
    console.log(protein, carbs, calories, fat, sugar);
